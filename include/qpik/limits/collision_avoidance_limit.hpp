...
struct CollisionAvoidanceLimit : public Limit {
  CollisionAvoidanceLimit(std::string name, Configuration &config,
                          CollisionPairs &collision_pairs, double gain = 0.85,
  ...
  void add_user_defined_geom(UserGeom &user_defined_geom);
  void add_user_defined_geom(std::string name, Eigen::Vector3d position,
                             double radius);
  ...
  std::map<std::string, UserGeom> user_defined_geoms;

  Contact _compute_contact_with_minimum_distance(Configuration &config,
                                                 const std::string &geom1_name,
                                                 const std::string &geom2_name);

  Constraint compute_qp_inequalities(Configuration &config, float dt) override;
};
