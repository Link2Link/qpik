...
struct AccelerationLimit : public Limit {
    AccelerationLimit(std::string name, Configuration &config, const std::map<std::string, double> &joint_acceleration_limits);
    // 缺省joint_acceleration_limits，所有关节使用默认的accelerationLimit属性, 为速度的50倍
    AccelerationLimit(std::string name, Configuration &config);
    // 默认所有关节加速度限制为max_acceleration
    AccelerationLimit(std::string name, Configuration &config, double max_acceleration);
