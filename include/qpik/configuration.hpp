...
#include "qpik/geometry.hpp"

namespace qpik {
namespace pin = pinocchio;

typedef std::map<std::string, GeometryVariant> CollisionObjectDict;
...
struct Configuration {
  ...
  Eigen::Matrix4d FK(std::string a_frame_name,
  ...
  Eigen::MatrixXd Ja(std::string frame_name); // WORLD Local aligned 雅可比矩阵
  Eigen::MatrixXd Jcom();
  Eigen::MatrixXd Jb_dq(std::string frame_name, int idx); // 求Jb对q_i的偏导
  Eigen::Vector3d com_position();                         // 质心位置
