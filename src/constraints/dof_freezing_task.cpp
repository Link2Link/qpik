...
namespace qpik {
DOFFreezingTask::DOFFreezingTask(std::string name, Configuration &config,
...
Eigen::VectorXd DOFFreezingTask::compute_error(Configuration &config,
                                               float dt) {
  return Eigen::VectorXd::Zero(this->indices.size());
}

Eigen::MatrixXd DOFFreezingTask::compute_jacobian(Configuration &config,
                                                  float dt) {
