...
RelativeFrameTask::RelativeFrameTask(std::string name, std::string from_frame,
                                     std::string to_frame,
...
Eigen::VectorXd RelativeFrameTask::compute_error(Configuration &config,
                                                 float dt) {
...
Eigen::MatrixXd RelativeFrameTask::compute_jacobian(Configuration &config,
                                                    float dt) {
  Eigen::Matrix4d T_from = config.FK(this->from_frame);
  Eigen::Matrix4d T_to = config.FK(this->to_frame);
  Eigen::Matrix4d T_from_to = utils::TransInv(T_from) * T_to;
