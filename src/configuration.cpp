...
void Configuration::update(Eigen::VectorXd q) {
  ...
  if (q.size() != this->nq) {
    std::cerr << "更新失败，传入q尺寸(" << q.size() << ")与自由度(" << this->nq
              << ")不一致" << std::endl;
    return;
  }
  this->q = q;
...
void Configuration::update_velocity(Eigen::VectorXd v) { this->v = v; }

void Configuration::update_kinematics() {
  pin::forwardKinematics(this->model_, this->data_, this->q);
  pin::jacobianCenterOfMass(this->model_, this->data_, this->q);
  pin::computeCentroidalMap(this->model_, this->data_, this->q);
...
Eigen::Vector3d Configuration::com_position() { return this->data_.com[0]; }
Eigen::Matrix4d Configuration::FK(std::string a_frame_name,
                                  std::string b_frame_name) {
...
Eigen::MatrixXd Configuration::Jb_dq(std::string frame_name, int idx) {
  ...
  temp_v(idx) = 1.0;
  pin::computeJointJacobiansTimeVariation(this->model_, this->data_, this->q,
                                          temp_v);
  Eigen::MatrixXd dJ_pin = Eigen::MatrixXd::Zero(6, this->model_.nv);
  pin::getFrameJacobianTimeVariation(this->model_, this->data_,
                                     this->model_.getFrameId(frame_name),
...
void Configuration::print_info() {
  ...
  std::cout
      << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
      << std::endl;
  std::cout << "关节最大速度: " << this->velocityLimit.transpose() << std::endl;
  std::cout
      << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
      << std::endl;
}

Eigen::MatrixXd Configuration::Jcom() { return this->data_.Jcom; }
...
void Configuration::load_collision_objects_from_urdf(
  ...
  for (const auto &link_name : collision_link_names) {
    ...
    if (!link->collision || !link->collision->geometry) {
      throw std::runtime_error("Link '" + link_name +
                               "' has no collision geometry.");
    }
    ...
    case urdf::Geometry::BOX: {
      ...
      geometry_box.size = Eigen::Vector3d(box->dim.x, box->dim.y, box->dim.z);
      this->collision_objects[link_name] = geometry_box;

      std::cout << "Loaded box geometry: " << link_name
                << " with size: " << geometry_box.size.transpose() << std::endl;
      break;
    }
    case urdf::Geometry::CYLINDER: {
      const urdf::Cylinder *cylinder =
          static_cast<const urdf::Cylinder *>(geom);
      GeometryCylinder geometry_cylinder;
      geometry_cylinder.radius = cylinder->radius;
      geometry_cylinder.height = cylinder->length;
      this->collision_objects[link_name] = geometry_cylinder;

      std::cout << "Loaded cylinder geometry: " << link_name
                << " with radius: " << geometry_cylinder.radius
                << " and height: " << geometry_cylinder.height << std::endl;
      break;
    }
    case urdf::Geometry::SPHERE: {
      const urdf::Sphere *sphere = static_cast<const urdf::Sphere *>(geom);
      GeometrySphere geometry_sphere;
      geometry_sphere.radius = sphere->radius;
      this->collision_objects[link_name] = geometry_sphere;

      std::cout << "Loaded sphere geometry: " << link_name
                << " with radius: " << geometry_sphere.radius << std::endl;
      break;
    }
    case urdf::Geometry::MESH: {
      const urdf::Mesh *mesh = static_cast<const urdf::Mesh *>(geom);
      GeometryMesh geometry_mesh;
      geometry_mesh.filename = mesh->filename;
      geometry_mesh.scale =
          Eigen::Vector3d(mesh->scale.x, mesh->scale.y, mesh->scale.z);
      this->collision_objects[link_name] = geometry_mesh;

      std::cout << "Loaded mesh geometry: " << link_name
                << " with filename: " << geometry_mesh.filename
                << " and scale: " << geometry_mesh.scale.transpose()
                << std::endl;
      break;
    }
    default:
      throw std::runtime_error("Collision geometry type not supported.");
    }
  }
}
} // namespace qpik